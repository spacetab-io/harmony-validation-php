...
class JsonTest extends StringTestCase
{
    ...
    public function testValidateFailsWhenPassingAnInvalidJsonString()
    {
        /** @var Result $result */
        $result = yield (new Json())->validate('{foo: "bar"}');

        $this->assertFalse($result->isValid());
        $this->assertSame('DataFormat.Json', $result->getFirstError()->getMessage());
    }

    public function testValidateReturnsTrueWhenPassingAValidJsonString()
    {
        /** @var Result $result */
        $result = yield (new Json())->validate('{"foo": "bar"}');
