...
class Version5Test extends StringTestCase
{
    /**
     * Version5Test constructor.
     *
    ...
    {
        parent::__construct($name, $data, $dataName, Version5::class);
    }

    /**
     * @dataProvider provideInvalidUuids
     */
    public function testValidateFailsWhenPassingAnInvalidV4UuidString(string $uuid): Generator
    {
        /** @var Result $result */
        $result = yield (new Version5())->validate($uuid);

        $this->assertFalse($result->isValid());
        $this->assertSame('Uuid.Version5', $result->getFirstError()->getMessage());
    }

    /**
     * @dataProvider provideValidUuids
     */
    public function testValidateSucceedsWhenPassingAValidV4UuidString(string $uuid): Generator
    {
        /** @var Result $result */
        $result = yield (new Version5())->validate($uuid);
